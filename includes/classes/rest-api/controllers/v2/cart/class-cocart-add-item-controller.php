	...
	public function add_to_cart( $request = array() ) {
		try {
			...
			if ( 'variable' === $add_to_cart_handler || 'variation' === $add_to_cart_handler ) {
				 ...
				 * @param WP_REST_Request $request                The request object.
				 */
				$item_added_to_cart = apply_filters( "cocart_add_to_cart_handler_{$add_to_cart_handler}", $adding_to_cart, $request ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
			} else {
			...
			if ( ! is_wp_error( $item_added_to_cart ) ) {
				...
				if ( isset( $request['return_item'] ) && is_bool( $request['return_item'] ) && $request['return_item'] ) {
					$response = $controller->get_item( $item_added_to_cart['data'], $item_added_to_cart, $item_added_to_cart['key'] );
				} else {
