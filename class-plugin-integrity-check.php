 ...
 */
abstract class Plugin_Integrity_Check {
		...
		add_action( 'admin_notices', function () {
			if ( file_exists( $this->plugin_dir . $this->checksum_file_name ) ) {
				...
				if ( ! empty( $errors ) ) {
					...
					if ( is_array( $errors ) ) { ?>
						<p><strong><?php echo esc_html__( 'Warning:', $this->plugin_slug ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain ?></strong> <?php echo esc_html__( 'The following plugin files are either missing or do not match the expected checksum and may have been modified:', $this->plugin_slug ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain ?></p>
						<ul>
						...
						echo '<p><strong>' . esc_html__( 'Warning:', $this->plugin_slug ) . '</strong> ' . $errors . '</p>'; // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain, WordPress.Security.EscapeOutput.OutputNotEscaped
					}
					echo '</div>';
				}
	...
	public function plugin_checksum_missing_warning() {
		...
		<div class="notice notice-warning">
			<p><strong><?php echo esc_html__( 'Warning:', $this->plugin_slug ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain ?></strong> 
				<?php
				printf(
					/* translators: %s = Plugin name */
					esc_html__( 'The checksum file is missing for "%s". This may indicate the plugin has been altered. Please check your installation.', $this->plugin_slug ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain
	...
	public function plugin_activation_checksum_check() {
		...
		if ( ! file_exists( untrailingslashit( $this->plugin_dir ) . '/' . $this->checksum_file_name ) ) {
			...
			wp_die( esc_html__( 'Warning: For your security, the plugin did not activate because an important file is missing. This may indicate the plugin has been altered. Please contact support for help.', $this->plugin_slug ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain
		}
	...
	public function check_plugin_integrity_on_update( $upgrader, $hook_extra ) {
		...
		if ( 'plugin' === $hook_extra['type'] && ! empty( $hook_extra['result'] ) && is_array( $hook_extra['result'] ) ) {
			...
			if ( ! $plugin_found ) {
				...
				update_option( $this->plugin_integrity_notice,
					sprintf(
					/* translators: %s = Plugin name */
						__( 'There appears to be an issue matching the plugin installed for "%s".', $this->plugin_slug ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain
						$plugin_name
					)
				);
				return;
		...
		if ( ! file_exists( $this->plugin_dir . $this->checksum_file_name ) ) {
			update_option(
				...
				sprintf(
					/* translators: %s = Plugin name */
					__( 'The checksum file for "%1$s" is missing so we can\'t verify it\'s integrity. Please download %2$s from an official source.', $this->plugin_slug ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain
					$plugin_name
				)
			);
			return;
	...
	protected function validate_file_integrity() {
		...
		foreach ( $checksums as $file_path => $expected_hash ) {
			...
			if ( ! file_exists( $file_full_path ) ) {
				$errors[] = esc_html__( 'File missing:', $this->plugin_slug ) . ' ' . '<u>' . untrailingslashit( $file_full_path ) . '</u>'; // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain
				error_log( esc_html__( 'File missing:', $this->plugin_slug ) . ' ' . untrailingslashit( $file_full_path ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain
				continue;
			}
			...
			if ( $file_hash !== $expected_hash ) {
				$errors[] = sprintf(
					...
					__( 'There was a checksum mismatch for %1$s. Expected: %2$s got %3$s.', $this->plugin_slug ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain
					'<u>' . untrailingslashit( $file_full_path ) . '</u>',
				...
				error_log(
					sprintf(
						/* translators: 1 = File path, 2 = Expected hash, 3 = Found hash */
						esc_html__( 'There was a checksum mismatch for %1$s. Expected: %2$s got %3$s.', $this->plugin_slug ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralDomain
